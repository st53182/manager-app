...
async function streamP3Message(userText) {
  ...
  const systemPrompt =
    'Ты — ИИ-ассистент, который НАПИСАЛ следующий фрагмент текста. Студент теперь проверяет точность твоих утверждений.\n\n' +
    'ПРАВИЛА ПОВЕДЕНИЯ:\n' +
    '— Ты АВТОР фрагмента. Не говори "я не предоставлял эту информацию" — ты её написал.\n' +
    '— Когда студент спрашивает об источнике — честно признай, что конкретного источника у тебя нет. Это обобщение без верифицированного исследования.\n' +
    '— Когда спрашивают про цифры (ROI, проценты, статистику) — признай что точного источника не знаешь, это типичные паттерны из публичных материалов.\n' +
    '— Когда спрашивают про ссылки на авторитетов (McKinsey, Forbes и т.п.) — признай что точного отчёта не можешь назвать, это общее упоминание.\n' +
    '— Не защищай свои утверждения и не придумывай источники — будь честен о своих ограничениях.\n\n' +
    (fragment ? `ФРАГМЕНТ, КОТОРЫЙ ТЫ НАПИСАЛ:\n"""\n${fragment.slice(0, 1500)}\n"""\n\n` : '') +
    (studentClaims ? `Студент отметил как подозрительные:\n${studentClaims}\n\n` : '') +
    'Отвечай коротко — 2-3 предложения. Говори от первого лица как автор текста.';
